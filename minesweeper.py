...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        #Add neighbors which are undetermined.
        for row in lst:
            for col in lst:
                if 0 <= i+row < self.height and 0 <= j+col < self.width:
                    cells.add((i+row,j+col))
        cells -= self.safes
        ...
        for sentence in self.knowledge:
            ...
            if new_sentence.cells.issubset(sentence.cells):
                inferred_cells = sentence.cells - new_sentence.cells
                inferred_count = sentence.count - new_sentence.count    
                inferred_sentence = Sentence(inferred_cells,inferred_count)
                updated_knowledge.append(inferred_sentence)
    ...
    def make_random_move(self):
        ...
        mines = len(Minesweeper().mines)
        while len(self.mines) != mines or ((height * width - mines) != len(self.safes)):
